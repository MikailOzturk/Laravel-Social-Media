...
<div class="search-result new-job">
    <div class="badge"> <a href="{{URL::to('follow',$company->id)}}">
            <span>Takip Et</span>
    ...
    <div class="icon">
        ...
        @else
            <img src="uploads/images/company_images/{!! $company->image_url !!}" width="60" />
        @endif
    </div>
    <div class="content">
        <form action="{{url('showCompany')}}">
            <input name="comId" type="hidden" value="{{$company->id}}"/>
           <h2><input type="submit" value="{{$company->company_name}}" style="background: transparent"></h2>

        </form>

        <p>
            <span class="company"><a href="/job/1234">{{$company->sector}}</a></span>
            -
            <a href="/reviews/1234">
